...
def validate(data):
    ...
    for record in data:
        ...
        # Validation rules: price > 0 and category not empty
        if price > 0 and category:
            valid_records.append(record)
        else:
            error_count += 1
            if price <= 0:
                print(f"  [X] Record {record.get('id', 'unknown')}: Invalid price ({price})")
            if not category:
                print(f"  [X] Record {record.get('id', 'unknown')}: Empty category")

    print(f"Validation complete. {len(valid_records)} valid records, {error_count} dropped (invalid)")
...
def transform(data):
    ...
    print(f"Transform complete. {len(df)} records transformed with business logic applied.")
    return df
